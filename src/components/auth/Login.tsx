...
/* ✅ Guest credentials */
const GUEST_CREDENTIALS: LoginData = {
  email: 'nextdeal@gmail.com',
  password: 'asfaqurrahman055@gmail.com',
};
...
const LoginPage: React.FC = () => {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: async (data: LoginResponse) => {
      ...
      sessionStorage.setItem(
        'fromLogin',
        sessionStorage.getItem('fromLogin') === 'true' ? 'true' : 'false'
      );
      sessionStorage.setItem('user-status', 'true');

      if (data?.data) {
        loginUser(data.data as User, data?.token);
      }
  ...
  const handleGuestLogin = () => {
    formik.setValues(GUEST_CREDENTIALS);
    mutate(GUEST_CREDENTIALS);
  };
  ...
  return (
    ...
    >
      <Typography
        variant="h4"
        sx={{ fontSize: { xs: '32px', md: '44px' }, mb: 2 }}
      >
        {t('welcome')}
      </Typography>
      ...
      <form onSubmit={formik.handleSubmit} style={{ width: '100%' }}>
        ...
        <CustomButton onClick={() => formik.submitForm()}>
          {isPending ? <CircularProgress size={28} /> : t('login')}
        </CustomButton>
        ...
        <Typography sx={{ fontSize: '16px', fontWeight: 500, color: colors.textBlack, textAlign: 'center' }}>- or -</Typography>
        {/* ✅ Guest Login */}
        <Button
          sx={{
        ...
        >
          {isPending ? <CircularProgress size={24} /> : 'Continue as Guest'}
        </Button>
        </Box>
      </form>
